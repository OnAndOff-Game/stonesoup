...
{
    _mpr(korean_translate(text));
}

void mpr_nojoin(msg_channel_type channel, string text)
{
    _mpr(korean_translate(text), channel, 0, true);
}
...
{
    const string translated_format = korean_translate(format);
    format = translated_format.c_str();
    va_list ap;
    va_copy(ap, argp);
    char buff[200];
