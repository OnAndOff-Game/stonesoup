...
static TextDB AllDBs[] =
{
    ...
    // Exact, case-sensitive UI and message translations. Unlike description
    // keys, complete English sentences must not be folded to lowercase.
    TextDB("strings", "strings/", { "messages.txt" }, false),
};
...
static TextDB& StringsDB     = AllDBs[11];
...
TextDB::TextDB(const char* db_name, const char* dir, vector<string> files,
               bool canonicalise_keys)
    : _db_name(db_name), _directory(dir), _input_files(files),
      _canonicalise_keys(canonicalise_keys),
...
TextDB::TextDB(TextDB *parent)
    : _db_name(parent->_db_name),
      ...
      _canonicalise_keys(parent->_canonicalise_keys),
...
void TextDB::_regenerate_db()
{
    ...
    for (const string &file : _input_files)
    {
            ...
            || !_parent) // english is mandatory
        {
            snprintf(buf, sizeof(buf), ":%" PRId64, (int64_t)mtime);
            ts += buf;
            _store_text_db(full_input_path, _db, _canonicalise_keys);
        }
    }
...
static void _parse_text_db(LineInput &inf, DBM *db, bool canonicalise_keys)
{
    string key;
    string value;
...
static void _store_text_db(const string &in, DBM *db, bool canonicalise_keys)
{
    UTF8FileLineInput inf(in.c_str());
    if (inf.error())
        end(1, true, "Unable to open input file: %s", in.c_str());

    _parse_text_db(inf, db, canonicalise_keys);
}
...
{
    return _query_database(StringsDB, key, false, false);
}
