...
// DEBUG_GLOBALS already makes the global player indirect so that its lifetime
// can be controlled. Cooperative multiplayer uses the same proven access
// pattern for a different reason: the simulation selects which player actor
// is active while resolving that player's command.
#ifdef DEBUG_GLOBALS
extern player* real_you;
#define you (*real_you)
#elif defined(USE_MULTIPLAYER)
extern player* active_you;
#define you (*active_you)
#else
extern player you;
#endif
...
public:
    ...
    {
#if !defined(DEBUG_GLOBALS) && !defined(USE_MULTIPLAYER)
