define(["exports", "jquery", "key_conversion", "chat", "comm",
        ...
        "contrib/jquery.waitforimages", "contrib/inflate", "multiplayer_lobby"],
function (exports, $, key_conversion, chat, comm) {
    ...
    function auth_error(data)
    {
        ...
        if (reason)
            ...
            $("#login_message").html("로그인에 실패했습니다.");
    }
    ...
    function logged_in(data)
    {
        ...
        hide_prompt();
        $("#login_message").html(username + "(으)로 로그인했습니다");
    ...
    function admin_pw_reset()
    {
        ...
        if (username.length == 0)
            admin_log_msg("사용자 이름을 입력하지 않았습니다.");
        else
    ...
    {
        if (data.error)
            admin_log_msg("비밀번호 재설정 실패: " + data.error);
        else
        {
            admin_log_msg(data.username + "의 비밀번호 재설정 토큰을 설정했습니다");
            $("#ok_message_content").html("<div><span><b>비밀번호 재설정 정보:</b></span></div>");
            $("#ok_message_content").append("<div><span>사용자 이름: " + data.username + "</span></div>");
            $("#ok_message_content").append("<div><span>사용자 이메일: <tt>" + data.email + "</tt></span></div>");
            $("#ok_message_content").append("<div>보낼 메시지:<pre>" + data.email_body + "</pre></div>");
    ...
    function admin_pw_reset_clear()
    {
        ...
        {
            admin_log_msg(username + "의 비밀번호 재설정 토큰을 지웠습니다");
    ...
    {
        var text = '<span class="fg5">서버 전체 공지: </span><span>'
            + data.text + '</span>';
    ...
    function exit_reason_message(reason, watched_name)
    {
        if (watched_name)
        {
            ...
            case "dead":
                return null;
            case "cancel":
                return watched_name + "님이 캐릭터를 만들기 전에 종료했습니다.";
            case "saved":
                return watched_name + "님이 게임을 저장하고 종료했습니다.";
            case "crash":
                return watched_name + "님의 게임이 비정상 종료되었습니다.";
            case "error":
                return watched_name + "님의 게임이 오류로 종료되었습니다.";
            case "disconnect":
                return watched_name + "님의 연결이 끊겼습니다.";
            default:
                return watched_name + "님의 게임이 예기치 않게 종료되었습니다."
        ...
        else
        {
            ...
            case "cancel":
                return null;
            case "crash":
                return "게임이 비정상 종료되었습니다.";
            case "error":
                return "게임이 오류로 종료되었습니다.";
            case "disconnect":
                return "연결이 끊겼습니다.";
            default:
                return "게임이 예기치 않게 종료되었습니다."
                       + (reason != "unknown" ? " (" + reason + ")" : "");
            }
        }
    }
    ...
    function register()
    {
        ...
        {
            $("#register_message").html("사용자 이름에는 공백을 넣을 수 없습니다.");
            return false;
        }

        if (password !== password_repeat)
        {
            $("#register_message").html("비밀번호가 일치하지 않습니다.");
            return false;
        }
    ...
    {
        $("#ok_message_content").html("비밀번호를 변경했습니다.");
        show_dialog("#floating_ok_message");
    }
    ...
    function change_password()
    {
        ...
        {
            $("#chpw_message").html("비밀번호가 일치하지 않습니다.");
            return false;
        }
    ...
    function change_email_done(data)
    {
        if ( data.email == "" )
        {
            $("#ok_message_content").html("계정에서 이메일 주소를 제거했습니다.");
        }
        else
        {
            $("#ok_message_content").html("이메일 주소를 " + data.email + "(으)로 설정했습니다.");
        }
    ...
    function forgot_password()
    {
        ...
        if (email.indexOf(" ") >= 0)
        {
            $("#forgot_message").html("이메일 주소에는 공백을 넣을 수 없습니다.");
            return false;
        }
    ...
    function reset_password()
    {
        ...
        if (password !== password_repeat)
        {
            $("#reset_pw_message").html("비밀번호가 일치하지 않습니다.");
            return false;
        }
    ...
    function login_required(data)
    {
        cleanup();
        show_loading_screen();
        show_prompt("<span id='prompt_game'>"
                    + data.game + "</span>:",
                    "플레이하려면 로그인해야 합니다. <a href='#lobby'>로비로 돌아가기</a>");
    }
    ...
    function chat_login(data)
    {
        ...
        var a = $("<a href='javascript:'>닫기</a>");
        a.click(hide_prompt);
        show_prompt("채팅하려면 로그인하세요:", a);
    }
    ...
    $(document).ready(function () {
        ...
        if ("WebSocket" in window)
        {
            ...
            socket.onerror = function ()
            {
                if (!showing_close_message)
                {
                    set_layer("crt");
                    $("#crt").html("");
                    $("#crt").append("WebSocket 연결에 실패했습니다.<br>");
                    showing_close_message = true;
                }
            };
            ...
            socket.onclose = function (ev)
            {
                if (!showing_close_message)
                {
                    set_layer("crt");
                    $("#crt").html("");
                    $("#crt").append("WebSocket 연결이 종료되었습니다.<br>");
                    if (ev.reason)
                    {
                        $("#crt").append("이유:<br>" + ev.reason + "<br>");
                    }
                    $("#crt").append("새로고침하여 다시 시도하세요.<br>");
                    showing_close_message = true;
                }
            };
        }
        ...
        {
            set_layer("crt");
            $("#crt").html("이 브라우저는 WebSocket을 지원하지 않습니다.<br><br>");
            $("#crt").append("WebTiles를 실행할 수 있는 브라우저: ");
