...
def user_passwd_match(username, passwd):  # type: (str, str) -> Optional[Tuple[str, str]]
    ...
    if result and dgl_is_banned(result[2]):
        return False, result[0], '비활성화된 계정입니다.'
...
def register_user(username, passwd, email):  # type: (str, str, str) -> Optional[str]
    ...
    if config.get('new_accounts_disabled'):
        ...
        return "새 계정 생성이 비활성화되어 있습니다."

    if passwd == "":
        return "비밀번호를 비워 둘 수 없습니다."
...
def find_recovery_token(token):
    ...
    if lifetime < 1:
        return None, None, "복구 토큰이 비활성화되어 있습니다."
    ...
    if not result:
        return None, None, "잘못된 토큰입니다."
    else:
        userid = result[0]
        username = result[1]
        expired = result[2]
        if expired == 'Y':
            return userid, username, "만료된 토큰입니다."
        else:
...
def generate_token_email(token):
    ...
    msg_body_plaintext = """누군가(본인이기를 바랍니다) %s 계정의 비밀번호 재설정을 요청했습니다.

본인이 요청했다면 다음 링크에서 비밀번호를 재설정하세요:

    %s

이 링크는 %d시간 뒤 만료됩니다. 비밀번호 재설정을 요청하지 않았다면
이 이메일을 무시하셔도 됩니다.
""" % (lobby_url, url_text, config.get('recovery_token_lifetime'))
  ...
  <head></head>
  <body>
    <p>누군가(본인이기를 바랍니다) %s 계정의 비밀번호 재설정을 요청했습니다.<br /><br />
       본인이 요청했다면 다음 링크에서 비밀번호를 재설정하세요:<br /><br />
...
def generate_forgot_password(username):
    if not username:
        return False, "사용자 이름을 입력하지 않았습니다."
    ...
    lifetime = int(config.get('recovery_token_lifetime'))
    if lifetime < 1:
        return False, "복구 토큰이 비활성화되어 있습니다."
...
def send_forgot_password(email):  # type: (str) -> Tuple[bool, Optional[str]]
    ...
    """
    if not email:
        return False, "이메일 주소를 비워 둘 수 없습니다."
