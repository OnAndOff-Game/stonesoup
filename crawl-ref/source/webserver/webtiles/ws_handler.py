...
from webtiles.room_lobby import RoomDirectory, RoomLobbyError, RoomState
...
multiplayer_rooms = RoomDirectory()


def _room_socket(client_id):
    for socket in list(sockets):
        if getattr(socket, "room_client_id", None) == client_id:
            return socket
    return None


def _send_room_directory(socket):
    if config.get('multiplayer_lobby', False):
        socket.send_message("room_list", rooms=multiplayer_rooms.list_rooms())


def _broadcast_room_directory():
    if not config.get('multiplayer_lobby', False):
        return
    rooms = multiplayer_rooms.list_rooms()
    for socket in list(sockets):
        socket.send_message("room_list", rooms=rooms)


def _broadcast_room_state(room):
    for client_id in list(room.players) + list(room.spectators):
        socket = _room_socket(client_id)
        if socket:
            socket.send_message("room_state", room=room.private_for(client_id))


def _notify_room_closed(room, reason):
    for client_id in list(room.players) + list(room.spectators):
        socket = _room_socket(client_id)
        if socket:
            if socket.watched_game:
                socket.stop_watching()
            if not socket.is_running(partial=True):
                socket.send_message("go_lobby")
            socket.send_message("room_closed", room_id=room.room_id,
                                reason=reason)
...
def room_errors(f):
    """Turn room validation failures into safe, translated client messages."""
    def wrapper(self, *args, **kwargs):
        try:
            return f(self, *args, **kwargs)
        except RoomLobbyError as error:
            self.send_message("room_error", reason=str(error))
    return wrapper
...
class CrawlWebSocket(tornado.websocket.WebSocketHandler):
    def __init__(self, app, req, **kwargs):
        ...
        self.save_info = dict()
        self.room_client_id = secrets.token_urlsafe(9)
        self.room_display_name = "모험가-" + self.room_client_id[-4:]
        ...
        self.message_handlers = {
            ...
            "pong": self.pong,
            "watch": self.watch,
            "room_set_name": self.room_set_name,
            "room_refresh": self.room_refresh,
            "room_create": self.room_create,
            "room_join": self.room_join,
            "room_watch": self.room_watch,
            "room_start": self.room_start,
            "room_leave": self.room_leave,
            "room_signal": self.room_signal,
    ...
    def admin_announce(self, text):
        ...
        self.send_message("admin_log", text="공지를 보냈습니다 ('" + text + "')")
    ...
    def admin_pw_reset(self, username):
        ...
        if not user_info:
            self.send_message("admin_pw_reset_done", error="잘못된 사용자입니다")
            return
    ...
    def open(self):
        ...
        if config.get('multiplayer_lobby', False):
            self.send_message("room_hello", client_id=self.room_client_id,
                              display_name=self.room_display_name)

        self.reset_idle_timeouts()

        if config.get('max_connections') < len(sockets):
            self.append_message("connection_closed('최대 접속자 수에 도달했습니다. 잠시 후 다시 시도해 주세요.');")
            self.close()
        elif shutting_down:
            self.close()
        else:
            if config.get('dgl_mode'):
                ...
                self.send_lobby()
            elif config.get('multiplayer_lobby', False):
    ...
    def check_origin(self, origin):
        # A public dedicated server must not accept room commands from an
        # unrelated web origin using a victim's browser connection.
        if not origin:
            return True
    ...
    def send_lobby_data(self):
        ...
        self.queue_message("lobby_complete")
        if config.get('multiplayer_lobby', False):
            self.queue_message("room_list", rooms=multiplayer_rooms.list_rooms())
        self.flush_messages()
    ...
    def room_refresh(self):
        self.reset_lobby_timeout()
        _send_room_directory(self)

    @room_errors
    def room_set_name(self, name):
        from webtiles.room_lobby import normalize_display_name
        self.room_display_name = normalize_display_name(name)
        room = multiplayer_rooms.rename(
            self.room_client_id, self.room_display_name)
        self.send_message("room_name_set", display_name=self.room_display_name)
        if room:
            _broadcast_room_state(room)
            _broadcast_room_directory()

    @room_errors
    def room_create(self, title, settings=None):
        if self.is_running() or self.watched_game:
            raise RoomLobbyError("게임을 종료하고 로비에서 방을 만들어 주세요")
        room = multiplayer_rooms.create(
            self.room_client_id, self.room_display_name, title, settings)
        self.send_message("room_joined", room=room.private_for(
            self.room_client_id))
        _broadcast_room_state(room)
        _broadcast_room_directory()
    ...
    def room_join(self, room_id):
        if self.is_running() or self.watched_game:
            raise RoomLobbyError("게임을 종료하고 로비에서 참가해 주세요")
        room = multiplayer_rooms.join(
            room_id, self.room_client_id, self.room_display_name)
        self.send_message("room_joined", room=room.private_for(
            self.room_client_id))
        _broadcast_room_state(room)
        _broadcast_room_directory()
    ...
    @room_errors
    def room_watch(self, room_id):
        if self.is_running():
            raise RoomLobbyError("플레이 중에는 다른 방을 관전할 수 없습니다")
        room = multiplayer_rooms.join(
            room_id, self.room_client_id, self.room_display_name,
            spectator=True)
        self.send_message("room_joined", room=room.private_for(
        ...
        if room.state == RoomState.RUNNING:
            self._watch_room_process(room)

    def _watch_room_process(self, room):
        host = _room_socket(room.host_id)
        if not host or not host.process or self.room_client_id == room.host_id:
            return False
        if self.watched_game and self.watched_game != host.process:
            self.stop_watching()
        if self.watched_game != host.process:
            self.watched_game = host.process
            host.process.add_watcher(self)
            self.send_message("watching_started", username=room.title)
        return True

    def _attach_room_members_when_ready(self, room_id, attempt=0):
        room = multiplayer_rooms.rooms.get(room_id)
        if (not room or room.host_id != self.room_client_id or
                not self.process):
            return
        if (not self.process.cached_options_message and attempt < 20):
            IOLoop.current().add_timeout(
                time.time() + 0.1,
                lambda: self._attach_room_members_when_ready(
                    room_id, attempt + 1))
            return
        for client_id in list(room.players) + list(room.spectators):
            ...
            socket = _room_socket(client_id)
            if socket:
                socket._watch_room_process(room)

    @room_errors
    def room_start(self):
        room = multiplayer_rooms.start(self.room_client_id)
        for client_id in list(room.players) + list(room.spectators):
            socket = _room_socket(client_id)
            if socket:
                socket.send_message("room_started", room_id=room.room_id,
                                    role=room.role_of(client_id).value,
        ...
        self.start_crawl(None)
        if not self.process:
            multiplayer_rooms.close(self.room_client_id)
            _notify_room_closed(room, "게임 프로세스를 시작하지 못했습니다")
            _broadcast_room_directory()
            return

        # Participants other than the host receive the authoritative WebTiles
        # stream as watchers. Their control channel to the host is WebRTC P2P.
        self._attach_room_members_when_ready(room.room_id)

    @room_errors
    def room_leave(self):
        room = multiplayer_rooms.room_for_client(self.room_client_id)
        if room is None:
            raise RoomLobbyError("참가 중인 방이 없습니다")
        was_host = room.host_id == self.room_client_id
        was_running = room.state == RoomState.RUNNING
        room, closed = multiplayer_rooms.leave(self.room_client_id)

        if self.watched_game:
            self.stop_watching()
        self.send_message("room_left", room_id=room.room_id)

        if closed:
            if was_host and was_running and self.process:
                self.process.stop()
            _notify_room_closed(room, "방장이 나가서 방이 종료되었습니다")
        else:
            _broadcast_room_state(room)
        _broadcast_room_directory()

    @room_errors
    def room_signal(self, target_id, signal):
        room = multiplayer_rooms.validate_signal(
            self.room_client_id, target_id, signal)
        target = _room_socket(target_id)
        if not target:
            raise RoomLobbyError("상대 사용자의 연결이 종료되었습니다")
        target.send_message("room_signal", room_id=room.room_id,
                            from_id=self.room_client_id, signal=signal)
    ...
    def start_crawl(self, game_id):
        room = None
        if config.get('multiplayer_lobby', False) and not config.get('dgl_mode'):
            room = multiplayer_rooms.room_for_client(self.room_client_id)
            if (room is None or room.host_id != self.room_client_id or
                    room.state != RoomState.RUNNING):
                self.send_message("room_error",
                    reason="실행 중인 방의 방장만 게임을 시작할 수 있습니다")
                return
        ...
        if config.get('dgl_mode'):
            ...
            self.process = process_handler.DGLLessCrawlProcessHandler(
                ...
                room_settings=room.settings if room else None)
        ...
        try:
            ...
            self.process.handle_notification(self.process.username,
                            "사용 가능한 채팅 명령은 '/help'에서 확인하세요")
            if not config.get('max_chat_length'):
                self.process.handle_notification(self.process.username,
                            "이 서버에서는 일반 채팅이 비활성화되어 있습니다")
    ...
    def _on_crawl_end(self):
        ended_room = multiplayer_rooms.room_for_client(self.room_client_id)
        close_multiplayer_room = bool(
            ended_room and ended_room.host_id == self.room_client_id and
            ended_room.state == RoomState.RUNNING)
        if self.process:
            if config.get('dgl_mode'):
        ...
        if close_multiplayer_room:
            multiplayer_rooms.close(self.room_client_id)
            _notify_room_closed(ended_room, "게임이 종료되었습니다")
            _broadcast_room_directory()
            ...
            if shutting_down:
                ...
                if config.get('dgl_mode'):
                    if not self.watched_game:
                        self.send_message("go_lobby")
                        self.send_lobby()
                elif not config.get('multiplayer_lobby', False):
                    self.start_crawl(None)
    ...
    def do_login(self, username):
        ...
        self.room_display_name = username
        room = multiplayer_rooms.rename(self.room_client_id, username)
        if room:
            _broadcast_room_state(room)
            _broadcast_room_directory()
        ...
        def login_callback(result):
            success = result == 0
            if not success:
                msg = ("rc 및 사망 기록을 초기화하지 못했습니다.<br>" +
                       "서버 설정에 문제가 있을 가능성이 큽니다.")
                self.send_message("close", reason = msg)
        ...
        if receiver:
            if self.account_restricted():
                self.send_message("chat",
                        content='제한된 계정에서는 채팅할 수 없습니다')
                return
            if self.username is None:
                self.send_message("chat",
                        content='메시지를 보내려면 로그인해야 합니다.')
                return
    ...
    def change_password(self, cur_password, new_password):
        ...
        if not self.update_db_info():
            self.send_message("change_password_fail", reason="비활성화된 계정입니다")
            return

        if self.username is None:
            self.send_message("change_password_fail",
                    reason="비밀번호를 변경하려면 로그인해야 합니다.")
            return

        if self.account_restricted():
            self.send_message("change_password_fail",
                    reason="제한된 계정에서는 비밀번호를 변경할 수 없습니다.")
            return

        if not userdb.user_passwd_match(self.username, cur_password)[0]:
            self.send_message("change_password_fail", reason = "현재 비밀번호가 일치하지 않습니다.")
    ...
    def change_email(self, email):
        ...
        if not self.update_db_info():
            self.send_message("change_email_fail", reason="비활성화된 계정입니다")
            return

        if self.username is None:
            self.send_message("change_email_fail", reason = "이메일을 변경하려면 로그인해야 합니다")
            return
        if self.account_restricted():
            self.send_message("change_email_fail",
                    reason="제한된 계정에서는 이메일을 변경할 수 없습니다.")
            return
    ...
    def go_lobby(self, message=None):
        if (not config.get('dgl_mode') and
                not config.get('multiplayer_lobby', False)):
            return
    ...
    def on_close(self):
        ...
        if room:
            if closed:
                _notify_room_closed(room, "방장의 연결이 종료되어 방이 닫혔습니다")
            else:
                _broadcast_room_state(room)
            _broadcast_room_directory()
