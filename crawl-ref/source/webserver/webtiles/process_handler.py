...
class CrawlProcessHandlerBase(object):
    def __init__(self, game_params, username, logger):
        ...
        self.cached_options_message = None
    ...
    def handle_process_message(self, msg, send): # type: (str, bool) -> None
        ...
        # A spectator may load the game client after Crawl's initial options
        # message was broadcast. Keep the latest options so late room viewers
        # never render a layout with uninitialised colours or tile sizes.
        if re.search(r'"msg" *: *"options"', msg[0:80]):
            self.cached_options_message = msg
    ...
    def chat_command_help(self, source):
        ...
        self.handle_notification(source, "사용 가능한 채팅 명령:")
        self.chat_help_message(source, "/help", "채팅 명령 도움말 보기")
        self.chat_help_message(source, "/hide", "채팅 창 숨기기")
        if self.is_player(source):
            self.chat_help_message(source, "/kick <이름> [<분>]", "해당 관전자를 일정 시간 내보내기")
            self.chat_help_message(source, "", "기본값: 15분. 다음 접속에는 유지되지 않습니다.")
            self.chat_help_message(source, "/block <이름>", "해당 관전자를 내보내고 차단")
            self.chat_help_message(source, "", "채널에 있는 이름만 가능. 특수 이름:")
            self.chat_help_message(source, "", "[anon] 익명 관전 비활성화")
            self.chat_help_message(source, "", "[all] 모든 관전 비활성화")
            self.chat_help_message(source, "/blocklist", "전체 차단 목록 보기")
            self.chat_help_message(source, "/unblock <이름>", "차단 목록에서 제거")
            self.chat_help_message(source, "/unblock *", "차단 목록 비우기")
        ...
        for w in self._receivers:
            if self.is_blocked(w.username) and not w.is_admin():
                IOLoop.current().add_callback(
                    lambda w=w: w.go_lobby(message="이 플레이어의 게임은 관전할 수 없습니다."))
    ...
    def restore_blocklist(self, source, l):
        ...
        self.handle_notification(source, "차단 목록을 복원합니다.")
        self.show_block_list(source)
    ...
    def kick(self, source, target_args):
        ...
        if (source == target):
            self.handle_notification(source, "자기 자신을 내보낼 수 없습니다.")
            return False
        player_name, watchers = self.get_watchers()
        ...
        if target in watchers:
            ...
            self.handle_notification(source,
                "관전자 '%s'님을 %d분 동안 내보냈습니다." % (target, interval))
        else:
            self.handle_notification(source, "내보낼 관전자를 지정하세요.")
            return False
        ...
        for w in receivers:
            if not w.is_admin():
                IOLoop.current().add_callback(
                    lambda w=w: w.go_lobby(message="게임에서 퇴장되었습니다."))
        return True
    ...
    def block(self, source, target):
        if not self.is_player(source):
            ...
            return False
        if (source == target):
            self.handle_notification(source, "자기 자신을 차단할 수 없습니다.")
            return False
        player_name, watchers = self.get_watchers()
        watchers = set(watchers)
    ...
    def unblock(self, source, target):
        if not self.is_player(source):
            self.handle_notification(source,
                            "관전자 차단을 해제할 권한이 없습니다.")
            return False
        if (source == target):
            self.handle_notification(source,
                                    "자기 자신의 차단 상태를 바꿀 수 없습니다.")
            return False
        if target == "*":
            if len(self.kicked):
                self.kicked = {}
                self.handle_notification(source, "퇴장 목록을 지웠습니다.")
            if (len(self.blocked) == 0):
                self.handle_notification(source, "차단된 사용자가 없습니다.")
                return False
            self.logger.info("Player '%s' has cleared their blocklist." % (source))
            self.handle_notification(source, "차단 목록을 비웠습니다.")
            self.blocked = set()
            self.save_blocklist(source)
            self.update_watcher_description()
            return True
        ...
        if target in self.kicked:
            ...
            self.handle_notification(source, "'%s'님을 퇴장 목록에서 제거합니다" % target)
            did_something = True

        if not target in self.blocked:
            if not did_something:
                self.handle_notification(source, "차단을 해제할 관전자를 지정하세요.")
            return False

        # these messages are more or less ok for [all], [anon]
        self.logger.info("Player '%s' has unblocked '%s'" % (source, target))
        self.handle_notification(source, "'%s'님의 차단을 해제했습니다." % target)
        self.blocked -= {target}
        self.save_blocklist(source)
        self.update_watcher_description()
        return True
    ...
    def show_block_list(self, source):
        ...
        if len(names) == 0:
            ...
            self.handle_notification(source, "차단한 사용자: " +
                                                            ", ".join(names))
        return True
    ...
    def add_watcher(self, watcher):
        ...
        if self.client_path:
            ...
            if watcher.watched_game == self:
                if self.cached_options_message:
                    watcher.append_message(self.cached_options_message, True)
                else:
    ...
    def send_client_to_all(self):
        for receiver in self._receivers:
            ...
            if receiver.watched_game == self:
                if self.cached_options_message:
                    receiver.append_message(self.cached_options_message, True)
                else:
...
class DGLLessCrawlProcessHandler(CrawlProcessHandler):
    def __init__(self, logger, room_id=None, room_settings=None):
        # used when dgl_mode = False
        safe_room_id = re.sub(r"[^a-zA-Z0-9_-]", "", room_id or "local")
        username = "room-" + safe_room_id
        self.room_settings = dict(room_settings or {})
        self.room_data_path = os.path.abspath(os.path.join(
            ".webtiles-rooms", safe_room_id))
    ...
    def _base_call(self):
        call = ["./crawl", "-dir", self.room_data_path,
                "-morgue", self.room_data_path]
        mode = self.room_settings.get("game_mode", "standard")
        seed = self.room_settings.get("seed", "")
        if mode == "sprint":
            call.append("-sprint")
        elif mode == "descent":
            call.append("-descent")
