...
function ($, exports, comm, client, key_conversion, dungeon_renderer, display,
    ...
    function layout(params, force)
    {
        ...
        var mobile_layout = window.matchMedia(
            "(max-width: 900px), (pointer: coarse) and (max-width: 1200px)"
        ).matches;
        $(document.body).toggleClass("mobile-layout", mobile_layout);
        ...
        // On phones the full sidebar becomes a drawer. It still needs a real
        // width for internal layout and minimap rendering, but must not steal
        // almost the entire viewport from the dungeon.
        if (mobile_layout)
            stat_width_px = Math.min(360, Math.max(280, window_width * 0.86));
        ...
        var mobile_controls_height = mobile_layout
            ? ($("#mobile-controls").outerHeight() || 0) : 0;
        var remaining_width = mobile_layout
            ? window_width : window_width - stat_width_px;
        ...
        $("#mobile_input input").width(Math.max(0, remaining_width-12));
    ...
    function mobile_input_focus_style(focused)
    {
        ...
        if (focused)
        {
            mi.attr("placeholder", "키보드를 닫으려면 여기를 누르세요");
            mi.css("background", "rgba(100, 100, 100, 0.5)");
        }
        else
        {
            mi.attr("placeholder", "여기를 눌러 키보드로 입력");
